...
import useAddessaddProjectStore from "src/store/dashboard/addProject";
...
const AddProject = () => {
  ...
  const { addProject, addProjectResponse } = useAddessaddProjectStore(
    (state) => state
  );
                ...
                render={({ field }) => (
                  <FormItem className="w-full dark:text-gray-950">
                    <FormLabel className="flex items-start justify-start">
                      {" "}
                      {t("workspace.project_name")}
                    </FormLabel>
                    <FormControl>
                      <Input
                        placeholder={t("workspace.placeholder_project_name")}
                        {...field}
                      />
                    </FormControl>
                    {/*    <FormDescription>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem className="w-full dark:text-gray-950">
                    ...
                    <Select
                      onValueChange={(value) => {
                        field.onChange(value); // met à jour react-hook-form
                      }}
                      value={field.value}
                    >
                      <FormControl>
                        <SelectTrigger>
                          <SelectValue
                            placeholder={t("workspace.placeholder_workspace")}
                          />
                        </SelectTrigger>
                      </FormControl>
                      <SelectContent>
