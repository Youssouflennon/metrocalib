...
const Add = () => {
  ...
  const [selectIdGroup, setSelectIdGroup] = useState<any>("");
  ...
  const { AllPermission, loadingAllPermission, fetchAllPermission } =
    useStoreAllPermission();

  const { AllGroup, loadingAllGroup, fetchAllGroup } = useStoreAllGroup();

  useEffect(() => {
    fetchAllGroup({ page, page_size: 123, search });
  }, [page, search, fetchAllGroup]);

  useEffect(() => {
    fetchAllPermission({ page, page_size: 156, search });
  }, [page, search, fetchAllPermission]);
  ...
  const { addUser, addUserResponse } = useAddessaddUserStore((state) => state);
  ...
  const onSubmit = async (data: z.infer<typeof FormSchema>) => {
    ...
    selectGroups.forEach((perm) =>
      formData.append("groups", String(perm.value))
    );
    selectedPermissions.forEach((perm) =>
      formData.append("user_permissions", String(perm.value))
    );
    try {
      // Send the FormData with an HTTP client (axios or fetch)
      await addUser(formData); // Assuming `addUser` can handle FormData
        ...
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
          <div className="grid grid-cols-2 gap-10 p-6">
            ...
            <FormField
              ...
              name="password"
              render={({ field }) => (
                <FormItem className="dark:text-[#1d0553]">
                ...
                <FormItem className="w-full dark:text-[#1d0553]">
                  ...
                  {preview && (
                    <img
                      src={preview}
                      alt="Aperçu"
                      className="mt-2 h-12 w-20 object-cover rounded-md border"
                    />
                  )}
                  <FormMessage />
                </FormItem>
            ...
            <FormField
              ...
              name="is_active"
              render={({ field }) => (
                <FormItem className="flex items-center justify-between rounded-lg border p-3 shadow-md bg-white dark:bg-[#1d0553]">
                  <div>
